...
public:
	...
	//Sprite functions
	Sprite& createSprite(u64 id, u32 typeID, u32 groupID = 0, u32 shaderID = Spring::baseShaderID);
	Sprite& getSprite(u64 id);
	const Sprite& getSprite(u64 id) const;
	bool containsSprite(u64 id) const;
	void destroySprite(u64 id);
	void destroyAllSprites();

	//Type functions
	void createType(u32 id, u32 textureID, const Vec2f& size);
	void createType(u32 id, u32 textureID, const Vec2f& size, const Vec2f& origin, const Vec2f& uvBase = Vec2f(0), const Vec2f& uvScale = Vec2f(1), SpriteOutline outline = SpriteOutline::Rectangle, const std::span<const u8>& polygon = {});
	bool hasType(u32 id) const;
	const SpriteType& getType(u32 id) const;
