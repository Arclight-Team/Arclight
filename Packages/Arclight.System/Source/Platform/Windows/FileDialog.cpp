...
namespace System {
	...
	void FileDialog::setDirectory(const Path& path) {
		...
		if (!SUCCEEDED(SHCreateItemFromParsingName(System::String::toUTF16(str).c_str(), nullptr, IID_IShellItem, &pItem))) {
	...
	void FileDialog::setTitle(const std::string& title) {
		...
		if (!SUCCEEDED(handle->dialog->SetTitle(System::String::toUTF16(title).c_str()))) {
	...
	void FileDialog::setButtonText(const std::string& text) {
		...
		if (!SUCCEEDED(handle->dialog->SetOkButtonLabel(System::String::toUTF16(text).c_str()))) {
	...
	void FileDialog::setFilenameLabel(const std::string& text) {
		...
		if (!SUCCEEDED(handle->dialog->SetFileNameLabel(System::String::toUTF16(text).c_str()))) {
	...
	void FileDialog::setInitialFilename(const std::string& filename) {
		...
		if (!SUCCEEDED(handle->dialog->SetFileName(System::String::toUTF16(filename).c_str()))) {
	...
	void FileDialog::setDefaultExtension(const std::string& ext) {
		...
		if (!SUCCEEDED(handle->dialog->SetDefaultExtension(System::String::toUTF16(ext).c_str()))) {
	...
	void FileDialog::setFileFilters(const std::vector<FileFilter>& filters, SizeT selected) {
		...
		for(const FileFilter& filter : filters) {
			...
			wideStrings.emplace_back(System::String::toUTF16(filter.text));
			wideStrings.emplace_back(System::String::toUTF16(fullPattern));
