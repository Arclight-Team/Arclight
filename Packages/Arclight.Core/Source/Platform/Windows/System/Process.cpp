...
namespace System {
		...
		*/
		void enumerateProcesses(const EnumerateProcessCallback& callback) {
			...
			if (!snapshot.valid()) {
				...
				LogE("Process") << System::getSystemErrorMessage();
	...
	bool Process::start(const ProcessStartInfo& startInfo) {
		...
		if (startInfo.moduleAsArgv0) {
			...
			commandLine = System::String::toUTF16(fullCMDL);

		} else {

			commandLine = System::String::toUTF16(startInfo.arguments);
		...
		if (!startInfo.environment.empty()) {
			...
			for (const std::string& s : startInfo.environment) {
				...
				envSliced.emplace_back(System::String::toUTF16(s));
	...
	Path Process::getExecutablePath(HandleT handle) {
		...
		return Path(System::String::toUTF8(filename));
