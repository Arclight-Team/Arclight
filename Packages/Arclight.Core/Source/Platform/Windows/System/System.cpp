...
std::optional<std::string> System::Environment::getVariable(const std::string& var) {
...
bool System::Environment::setVariable(const std::string& var, const std::string& contents) {
...
bool System::restart(bool force) {
...
bool System::shutdown(bool force) {
...
System::Process System::invoke(const std::string& command, const Path& cwd, bool enableRedirection) {
	...
	if (!p.start(info)) {
		LogE("System") << "Failed to invoke command '" << command << "'";
	}
...
bool System::launch(LaunchAction action, const Path& path, const std::string& params, bool switchCWD) {
