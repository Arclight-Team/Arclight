...
void BitmapDecoder::decodeDirect() {
	...
	u32 rowBytesAligned = Bits::ceilPowerOf2(rowBytes, 4);
...
void BitmapDecoder::decodeIndexed() {
	...
	u32 bitsPerRowAligned = Bits::ceilPowerOf2(bitsPerRow, 32);
	...
	auto loadData = [&]<u32 BPP>() {
		...
		for (u32 y = 0; y < height; y++) {
			...
			for (u32 i = 0; i < Bits::ceilPowerOf2(leftoverPixels, 8) / 8; i++) {
