...
public:
	...
	static Path getSaveFilePath(const std::string& title = std::string(), const Path& dir = Path(), const std::vector<FileFilter>& filters = {}, SizeT selected = 0, Options options = Options::Validate);
...
private:
	...
	static Path showAndReturnItem(Type type, const std::string& title, const Path& dir, const std::vector<FileFilter>& filters, SizeT selected, Options options);
