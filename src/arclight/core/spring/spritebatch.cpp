...
void SpriteBatch::createSprite(u64 id, const Mat3f& affine) {
	...
	setSpriteMatrix(id, affine);

}



void SpriteBatch::setSpriteTranslation(u64 id, const Vec2f& position) {

	arc_assert(offsets.contains(id), "Illegal incomplete sprite batch update");

	alignas(16) u8 flatVec[8];
	vectorDecay(position, flatVec);

	SizeT offset = offsets[id] + 16;

	std::copy_n(flatVec, dataSize, &vertexData[offset]);
...
void SpriteBatch::setSpriteMatrix(u64 id, const Mat3f& affine) {

	alignas(16) u8 flatMat[dataSize];
	matrixDecay(affine, flatMat);

	SizeT offset = offsets[id];

	std::copy_n(flatMat, dataSize, &vertexData[offset]);

	updateBounds(offset, dataSize);
...
void SpriteBatch::matrixDecay(const Mat3f& matrix, const std::span<u8>& dest) {
