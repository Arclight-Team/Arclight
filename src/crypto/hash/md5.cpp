...
constexpr static void dispatchBlock(const std::span<const u8>& data, u32& a0, u32& b0, u32& c0, u32& d0) noexcept {
...
Hash<128> MD5::hash(const std::span<const u8>& data) noexcept {
    ...
    if(construct.prevBlockUsed) {
        dispatchBlock({construct.prevLastBlock, 64}, a, b, c, d);
    }
    
    dispatchBlock({construct.lastBlock, 64}, a, b, c, d);
