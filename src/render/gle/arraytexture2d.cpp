...
void ArrayTexture2D::setData(u32 w, u32 h, u32 layers, TextureFormat format, TextureSourceFormat srcFormat, TextureSourceType srcType, void* data) {
	...
	glTexImage3D(getTextureTypeEnum(type), 0, getTextureFormatEnum(texFormat), w, h, layers, 0, getTextureSourceFormatEnum(srcFormat), getTextureSourceTypeEnum(srcType), data);

}



void ArrayTexture2D::setMipmapData(u32 level, TextureSourceFormat srcFormat, TextureSourceType srcType, void* data) {
	...
	glTexImage3D(getTextureTypeEnum(type), level, getTextureFormatEnum(texFormat), getMipmapSize(level, width), getMipmapSize(level, height), depth, 0, getTextureSourceFormatEnum(srcFormat), getTextureSourceTypeEnum(srcType), data);
...
void ArrayTexture2D::update(u32 x, u32 y, u32 w, u32 h, u32 layerStart, u32 layerCount, TextureSourceFormat srcFormat, TextureSourceType srcType, void* data, u32 level) {
	...
	if ((x + w) > getMipmapSize(level, width)) {
		GLE::error("Updating 2D array texture out of bounds: width = %d, requested: x = %d, w = %d", getMipmapSize(level, width), x, w);
		return;
	}

	if ((y + h) > getMipmapSize(level, height)) {
		GLE::error("Updating 2D array texture out of bounds: height = %d, requested: y = %d, h = %d", getMipmapSize(level, height), y, h);
		return;
	}

	if ((layerStart + layerCount) > depth) {
		GLE::error("Updating 2D array texture out of bounds: array size = %d, requested: start = %d, count = %d", height, layerStart, layerCount);
		return;
	}
